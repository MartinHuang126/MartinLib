...
        private static readonly string _ConnString = string.Empty;
        ...
                /// </summary>
        static MSSQLDBHelper()
        {
            try
            {
                if (string.IsNullOrEmpty(_ConnString))
                {
                    _ConnString = ConfigurationManager.ConnectionStrings["Conn_Engine"].ToString();
                }
            }
            catch (Exception e)
            {
        ...
        public static int InsertByBulk(DataTable dt, string dtName)
        {
            ...
            try
            {
                Log2Txt.logInfo(Log2Txt.GetMethodInfo() + "dtName: " + dtName);
                using (SqlConnection conn = new SqlConnection(_ConnString))
                {
                    OpenConnection(conn);
        ...
        public static int GetCountByProcedure(string cmdText, params SqlParameter[] sqlParams)
        {
            ...
            try
            {
                Log2Txt.logInfo(Log2Txt.GetMethodInfo() + "sql: " + cmdText);
                using (SqlConnection conn = new SqlConnection(_ConnString))
                {
                    OpenConnection(conn);
                    using (SqlCommand cmd = conn.CreateCommand())
                    {
                        ...
                        if (sqlParams != null && sqlParams.Length > 0)
                            cmd.Parameters.AddRange(sqlParams);
                        count = cmd.ExecuteNonQuery(); //执行非查询命令,并返回受影响的行数
                        cmd.Parameters.Clear();
                        cmd.Dispose();
                    }
                }
        ...
                /// <param name="sqlParams">[可选]sql参数化</param>
                /// <returns>DataTable</returns>
        public static DataTable GetDataTableByProcedure(string cmdText, params SqlParameter[] sqlParams)
        {
            DataTable dt = null;
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(_ConnString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(cmdText, conn))
                    {
                        ...
                        {
                            cmd.Parameters.Clear();
                            dt = ConvertDataReaderToDataTable(dataReader);
...
                Log2Txt.logError(Log2Txt.GetMethodInfo() + e.Message);
        ...
        public static bool ExecSqlTran(ArrayList sqlStringList)
        {
            ...
            try
            {
                Log2Txt.logInfo(Log2Txt.GetMethodInfo() + "sql: " + sqlStringList.ToString());
                using (SqlConnection conn = new SqlConnection(_ConnString))
                {
                    OpenConnection(conn);
                    using (SqlCommand cmd = new SqlCommand())
...
                Log2Txt.logError(Log2Txt.GetMethodInfo() + e.Message);
        ...
        public static DataTable GetDataReader(string cmdText, params SqlParameter[] sqlParams)
        {
            DataTable dt = null;
            try
            {
        ...
        public static object ExecuteScalar(string cmdText, CommandType commandType, params SqlParameter[] sqlParams)
        {
